...
dotnet publish -c %_C% -o %_P%\WixToolset.MSBuild\ src\WixToolset.MSBuild || exit /b
dotnet publish -c %_C% -o %_P%\WixToolset.MSBuild\broken\net461\ -f net461 -r dne src\wix || exit /b

dotnet test -c %_C% src\test\WixToolsetTest.MSBuild || exit /b

dotnet pack -c %_C% src\dotnet-wix || exit /b
dotnet pack -c %_C% src\WixToolset.MSBuild || exit /b
