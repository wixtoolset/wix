...
)
{
    ...
    while (S_OK == (hr = WcaFetchRecord(hView, &hRec)))
    {
        ...
        ExitOnFailure(hr, "Failed to get Wix4HttpSslBinding.CertificateThumbprint");
        ...
        // XX TODO implement lookup
        if (!sczCertificateThumbprint || !*sczCertificateThumbprint)
        {
            hr = E_INVALIDARG;
            ExitOnFailure(hr, "Require a CertificateThumbprint value for Wix4HttpSslBinding '%ls'", sczId);
        }
