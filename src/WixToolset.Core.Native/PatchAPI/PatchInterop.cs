...
namespace WixToolset.Core.Native.PatchAPI
{
    using System;
    ...
    internal static class PatchInterop
    {
        ...
        internal static uint ParseHexOrDecimal(string source)
        {
        ...
        public static bool CreateDelta(
                string deltaFile,
                string targetFile,
                ...
                )
        {
            ...
            var countOldFiles = (uint)basisFiles.Length;
            ...
            var oldFileInfoArray = new PatchOldFileInfoW[countOldFiles];
            var newRetainOffsetArray = ((null == targetRetainOffsets) ? new string[0] : targetRetainOffsets.Split(','));
            for (uint i = 0; i < countOldFiles; ++i)
            {
                var ofi = new PatchOldFileInfoW();
                ofi.oldFileName = basisFiles[i];
                var ignoreLengthArray = ((null == basisIgnoreLengths[i]) ? new string[0] : basisIgnoreLengths[i].Split(','));
                var ignoreOffsetArray = ((null == basisIgnoreOffsets[i]) ? new string[0] : basisIgnoreOffsets[i].Split(','));
                var retainLengthArray = ((null == basisRetainLengths[i]) ? new string[0] : basisRetainLengths[i].Split(','));
                var retainOffsetArray = ((null == basisRetainOffsets[i]) ? new string[0] : basisRetainOffsets[i].Split(','));
                // Validate inputs
                ...
                if (0 != ignoreLengthArray.Length)
                {
                    ...
                    for (var j = 0; j < ignoreLengthArray.Length; ++j)
                    {
                        var ignoreRange = new PatchIgnoreRange();
                ...
                if (0 != newRetainOffsetArray.Length)
                {
                    ...
                    for (var j = 0; j < newRetainOffsetArray.Length; ++j)
                    {
                        var retainRange = new PatchRetainRange();
        ...
        {
            var flags = PATCH_OPTION_FAIL_IF_SAME_FILE | PATCH_OPTION_FAIL_IF_BIGGER | PATCH_OPTION_USE_LZX_BEST;
        ...
        internal class PatchAPIMarshaler : ICustomMarshaler
        {
            ...
            private readonly PatchAPIMarshaler.MarshalType marshalType;
            ...
            private IntPtr CreateMainStruct(int oldFileCount)
            {
                ...
                var native = Marshal.AllocCoTaskMem(nativeSize);
            ...
            private static IntPtr CreateArrayOfStringA(string[] managed)
            {
                ...
                var size = managed.Length * Marshal.SizeOf(typeof(IntPtr));
                var native = Marshal.AllocCoTaskMem(size);
            ...
            private static IntPtr CreateInterleaveMap(PatchInterleaveMap[] managed)
            {
                ...
                var native = Marshal.AllocCoTaskMem(managed.Length * Marshal.SizeOf(typeof(IntPtr)));
            ...
            private IntPtr MarshalPOD(PatchOptionData managed)
            {
                ...
                var native = this.CreateMainStruct(managed.oldFileSymbolPathArray.Length);
            ...
            private IntPtr MarshalPOFIW_A(PatchOldFileInfoW[] managed)
            {
                ...
                var native = this.CreateMainStruct(managed.Length);
            ...
            private static IntPtr MarshalPIRArray(PatchIgnoreRange[] array)
            {
                ...
                var native = Marshal.AllocCoTaskMem(array.Length * Marshal.SizeOf(typeof(PatchIgnoreRange)));
            ...
            private static IntPtr MarshalPRRArray(PatchRetainRange[] array)
            {
                ...
                var native = Marshal.AllocCoTaskMem(array.Length * Marshal.SizeOf(typeof(PatchRetainRange)));
            ...
            private void CleanUpPOD(IntPtr native)
            {
                ...
                {
                    for (var i = 0; i < GetOldFileCount(native); ++i)
                    {
