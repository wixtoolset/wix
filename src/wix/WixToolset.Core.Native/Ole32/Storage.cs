...
namespace WixToolset.Core.Native.Ole32
{
    ...
    internal class Storage : IDisposable
    {
        ...
        {
            var storage = NativeMethods.StgCreateDocfile(storageFile, (uint)mode, 0);
        ...
        {
            var storage = NativeMethods.StgOpenStorage(storageFile, IntPtr.Zero, (uint)mode, IntPtr.Zero, 0);
        ...
        {
            this.storage.OpenStorage(name, null, (uint)(StorageMode.Read | StorageMode.ShareExclusive), IntPtr.Zero, 0, out var subStorage);
