...
namespace WixToolset.Core.WindowsInstaller.Bind
{
    ...
    {
        public GetFileFacadesFromTransforms(IMessaging messaging, IEnumerable<SubStorage> subStorages, TableDefinitionCollection tableDefinitions)
        {
        ...
        public IEnumerable<SubStorage> SubStorages { get; }
        ...
        public void Execute()
        {
            ...
            var pairedTransforms = this.SubStorages.Where(s => s.Name.StartsWith("#")).ToDictionary(s => s.Name.Substring(1), s => s.Data);

            foreach (var substorage in this.SubStorages)
            {
                if (substorage.Name.StartsWith("#"))
                {
                    // Skip paired transforms.
                    continue;
                }

                var mainTransform = substorage.Data;
                var mainWixFiles = new RowDictionary<WixFileRow>(mainTransform.Tables["WixFile"]);
